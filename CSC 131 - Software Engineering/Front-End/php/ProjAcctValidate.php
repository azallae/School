<?php
require 'ConnectProjAcct.php'; // make sure connectDB exist to continue
...
if(isset($_POST['acc_name'])){ //if it's set a variable has been passed
	...
	if(!empty($acc_name)){
		$acct_name_command = mysql_query("SELECT COUNT(`projName`) FROM `project_info` WHERE `projName`='$acc_name'"); // search for element in the column projName
